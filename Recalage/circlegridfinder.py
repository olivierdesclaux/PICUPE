...
class CircleGridFinder:
    ...
    and OpenCV's cv2.findCirclesGrid(). When finished, contains a list of
    ...
    running : bool
        Indicates if find thread is still running
    finished : bool
        Indicates if find thread has found numGrids grids and exited
    ...
    -------
    start()
        Begin find() thread to search for circle grids
    stop()
        Close find() thread, even if all grids not found
    drawCircles(frames)
    ...
    def __init__(self, streams, streamTypes, circleDetectors, numGrids, secondsToSkip=2, numRows=9, numCols=15):
        ...
        self.objectCorners = 21.2 * np.array([[2 * (x % numRows) + np.floor(x / numRows) % 2, np.floor(x / numRows), 0]
    ...
    def start(self):
        ...
        if not self.running:
            self.running = True
            self.finished = False
            Thread(target=self.find, args=(), daemon=True).start()
        else:
            raise RuntimeError(
    ...
    def find(self):
        ...
        while self.running and len(self.objectPositions) < self.numGrids:
            ...
            for (frame, circleDetector, streamType) in zip(frames, self.circleDetectors, self.streamTypes):
                ...
                ret, pos = cv2.findCirclesGrid(frame, self.boardSize, flags=cv2.CALIB_CB_ASYMMETRIC_GRID, blobDetector=circleDetector.get())
                if not ret:
            ...
            if boardsFound:
                ...
                for (gridPosition, imagePositions) in zip(tempGridPositions, self.allImagePositions):
    ...
    def drawCircles(self, frames):
        ...
        None.
        """
        # Assumes if multiple frames, there are corresponding multiple circleDetectors and streamTypes
        for (frame, frameType, circleDetector, imagePosition) in zip(frames, self.streamTypes, self.circleDetectors,
